...
node-01:
	...
	ssh -F $(SSH_CONFIG) $@ /opt/bin/docker swarm init --advertise-addr "$(NODE_IP:node=$@):2377" | tee $(SWARM_CONFIG)
...
node-02 node-03:
	...
	$(eval TOKEN=$$(shell awk '/--token/{print $$$$2;exit;}' $(SWARM_CONFIG)))
